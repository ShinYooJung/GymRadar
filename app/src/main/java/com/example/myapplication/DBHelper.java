...
class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE TrainingCenter(ID NUM, NAME TEXT, COORDINATEX REAL, COORDINATEY REAL)");
    } // 생성과 동시에 진행되어야 하는 내용
    ...
    public void updateTC(Integer id, String name, float coordinateX, float coordinateY) {
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL("UPDATE TrainingCenter SET NAME = '"+name+"', COORDINATEX = " + coordinateX + ", COORDINATEY = " + coordinateY + "" + " WHERE ID = '" + id + "'");
        db.close();
    }
    ...
    public List<String> getResultTC() { // 읽기가 가능하게 DB 열기
        ...
        while (cursor.moveToNext()) {
            result.add("이름 :" + cursor.getString(1) + ", 좌표X : " + cursor.getString(2) + ", 좌표Y : " + cursor.getString(3) + "\n");
        }
        return result;
    }
}